...
# ADD
def pieceValue ( pi, Square=None):
    pi = pi.symbol()
    if Square is None :
        if pi>'a' and pi <'z':
            return -piece[pi.upper()]
        return piece[pi]
    if pi>'a' and pi <'z':
        pi = pi.upper()
        return -pst[pi][Square]

    return pst[pi][63-Square]

def valueMove (board,move):

    sqFrom = move.from_square
    sqTo = move.to_square
    pFrom , pTo = board.piece_at(sqFrom) , board.piece_at(sqTo)
    value = pieceValue(pFrom,sqTo) - pieceValue(pFrom,sqFrom)
    if pTo !=None:
        value -=pieceValue(pTo, sqTo)
        value -=pieceValue(pTo)
    return value
...
class Searcher(object):
    def __init__(self, game):
        ...
        self.currTime = None
        #ADD
        self.situation = SITUATION.BEGIN
        self.value= staticEval(self.game)
        self.moveValueArr = []
        #END
        #self.move
        #self.hashkey = 0
    ...
    def pushMove(self,move):
        self.moveValueArr.append(self.value)
        if  self.game.board.is_castling(move):
            ...
            #print ('normal')
            moveValue = valueMove (self.game.board,move)
            self.value+=moveValue
            self.game.board.push(move)

    def popMove (self,move):
        preValue = self.moveValueArr.pop()
        self.value =preValue
        self.game.board.pop()

    def staticEvalNew(self):
    ...
    def mvvLva(self, move):
        ...
        return (piece[pieceAtTo.symbol().upper()] << 3) - piece[
            self.game.board.piece_at(move.from_square).symbol().upper()]
    ...
    def quiescenceEval(self, alpha, beta):
        ...
        if self.game.board.is_check():
            ...
            #ADD
            if self.situation == SITUATION.END:
                board = self.game.board
                moveChoice = self.game.board.legal_moves
                legalMoves = []
                for move in moveChoice:
    ...
    def getBestMove(self):
        ...
        if self.situation == SITUATION.MID:
            map1 =self.game.board.piece_map()
            if len(map1)<10:
                #print('end')
                self.situation = SITUATION.END
        if self.situation == SITUATION.BEGIN and self.game.board.queens==0:
            #print ('mid')
            self.situation = SITUATION.MID
        timeGap = 6
        ...
        while time.time() <= self.futureTime:
            ...
            if thisMove != chess.Move.null():
                ...
                break
            eprint(str(time.time() - self.currTime) + " - Depth : " + str(depth), "Best Move", str(bestMove),
                   "Best Score", str(bestScore))
            #if time.time() - self.currTime > timeGap * 1 / 2:
    ...
    def pvSearch(self, alpha, beta, depthLeft, root=False):
        ...
        transpositionBestMove, transpositionScore, iCanReplaceThisTransposition = self.lookupTranspositionTable(
            thisHash, alpha, beta, depthLeft)
...
class SocratesGame(object):
    ...
    def moveAndReturnsSAN(self):
        ...
        self.searcher.pushMove(myMove)
        return sanOutStr

    def readInAndReturnsSAN(self):
        sanInStr = raw_input().strip()
        #self.board.push_san(sanInStr)
        move = self.board.parse_san(sanInStr)
        self.searcher.pushMove(move)
        return sanInStr
...
def test():
    game_new = SocratesGame()
    game_old  = socrates.SocratesGame()

    for i in range(200):
        if game_new.board.is_game_over():
            print ('game over ' , i , i%2)
        now = time.time()
        print ('Round %d'%i)
        if(i%2==0):
            move = game_new.searcher.getBestMove()

            #game_new.board.push(move)
            print ('move ' ,move)
            game_new.searcher.pushMove(move)
            game_old.board.push(move)
            print ('1 ',game_new.searcher.staticEvalNew(),'2 ',staticEval(game_new))
            print ('turn ','socrates_new')
        else:
            move = game_old.searcher.getBestMove()
            game_new.searcher.pushMove(move)
            game_old.board.push(move)
            print ('turn','socrates_old')
        print('cost' , time.time()-now)
        print (game_new.board)
        print('-------------------------')
    #game.run()